...
| `late_delivery_risk`        | `INT`            |     | This is a simple, pre-calculated flag (a column with only 1s and 0s). 1 means the delivery was late. 0 means the delivery was on-time or early.This column is almost certainly created using a simple rule: IF 'Days for shipping (real)' > 'Days for shipment (scheduled)' THEN 1 ELSE 0                           | `silver.late_delivery_risk`                                               |
